...
messageEncoder::messageEncoder(ConnectionHandler &connectionHandler1) :connectionHandler(&connectionHandler1) {
...
void messageEncoder::run() {
    while (!connectionHandler->shouldTerminate) {
        while (!connectionHandler->gotAmessage) {
        }
...
//        if (toSend == "0004") {
//            myMutex.lock();
//            messageDecoder dec(*connectionHandler, myMutex);
//            dec.run();
//            myMutex.unlock();
//
//        }
            connectionHandler->gotAmessage = false;
        }
    }
